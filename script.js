...
function playVideo() {
  ...
  // Show the wrapper first (removes display:none on iframe), THEN set src
  // so the browser registers the src assignment within an active user-gesture context
...
// i18n — all text taken verbatim from original site pages
let currentLang = 'nl';
const tr = {
  nl: {
  ...
  fr: {
    ...
    droomBody: `<p>Imaginez un monde dans lequel nous pouvons apprendre davantage avec et de la nature. Utiliser la sagesse qui s'y cache dans notre quotidien pour nous reconnecter à la nature et à nous-mêmes. Nous imaginions cela depuis un moment. Un place pour cœur.</p><p>Nous voulions nous éloigner d'un monde de plus en plus anonyme. Pour pouvoir étendre nos expériences dans le coaching de personnes et l'accompagnement de start-ups depuis un nouvel endroit.</p><p>Ce rêve bouillonne en nous, Frédérique et Ronald Horn, depuis quelques années. À la recherche de l'endroit idéal pour faire de notre rêve une réalité, nous avons acheté une maison dans un quartier calme de la France fin 2021.</p><p>Avec toutes les commodités à proximité, mais avec la tranquillité et l'inspiration de la nature. Dans ce nouvel emplacement, nous ressentons l'espace et la liberté à partir desquels nous poursuivons et développons nos activités. Nous vous proposons des parcours sur-mesure dans et avec la nature, où nous vous guidons par votre questionnement et vous participez aux projets.</p>`,
